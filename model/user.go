...
func (user *User) Insert(inviterId int) error {
	...
	if inviterId != 0 {
		if common.QuotaForInvitee > 0 {
			...
			RecordLog(user.Id, LogTypeSystem, fmt.Sprintf("use invitation code to give %s", logger.LogQuota(common.QuotaForInvitee)))
		}
		if common.QuotaForInviter > 0 {
			//_ = IncreaseUserQuota(inviterId, common.QuotaForInviter)
			RecordLog(inviterId, LogTypeSystem, fmt.Sprintf("invite user to give %s", logger.LogQuota(common.QuotaForInviter)))
...
func (user *User) ValidateAndFill() (err error) {
	...
	if !okay || user.Status != common.UserStatusEnabled {
		return errors.New("the username or password is incorrect, or the user has been banned")
	}
	return nil
}
